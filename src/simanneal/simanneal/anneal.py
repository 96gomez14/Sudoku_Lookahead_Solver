        ...
        def run(T, steps):
            ...
            for _ in range(steps):
                ...
                if states_dict == None:
                    ...
                    prevEnergy = lookahead_solver.compute_avg(energy_dict)
                states_dict.clear()
                energy_dict.clear()
                states_dict[0] = self
                energy_dict[0] = E
                lamed = lookahead_solver.compute_lamed(E, prevEnergy, T)
                tree_dict = lookahead_solver.choose_tree(lamed)
                lookahead_solver.fill_states(0, tree_dict, states_dict, lamed, self)
                lookahead_solver.evaluate_tree(states_dict, energy_dict)
                self.state, E = lookahead_solver.choose_state(0, tree_dict, states_dict, energy_dict, lamed)
                dE = E - prevEnergy
                if dE <= 0.0 or decimal.Decimal(-dE / T).exp() >= random.random():
                    accepts += 1
                    if dE < 0.0:
                        improves += 1
                index += 1
